...
_RESERVE_CALENDAR_DAYS: Mapping[int, Mapping[int, Sequence[int]]] = {
    2023: {
        1: (2, 7, 8),
        2: (16, 17, 20),
        3: (18, 25),
...
_TARGET_DATE_VALUES: Sequence[date] = tuple(
    date(year, month, day)
    for year, months in _RESERVE_CALENDAR_DAYS.items()
    for month, days in months.items()
    for day in days
)

TARGET_DATES: Sequence[date] = tuple(sorted(set(_TARGET_DATE_VALUES)))
