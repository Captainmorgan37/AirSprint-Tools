...
def _is_ocs_leg(row: Mapping[str, Any]) -> bool:
    flight_type = row.get("flightType") or row.get("flight_type")
    if flight_type is None:
        return False

    normalized = str(flight_type).strip().upper()
    return normalized == "OCS"


def _exclude_ocs_rows(rows: Sequence[Mapping[str, Any]]) -> tuple[List[Mapping[str, Any]], int]:
    filtered_rows = [row for row in rows if not _is_ocs_leg(row)]
...
def _handle_audit_fetch(
    ...
    *,
    ...
    normalized_rows, skipped_subcharter = filter_out_subcharter_rows(normalized_rows)
    normalized_rows, skipped_ocs = _exclude_ocs_rows(normalized_rows)
