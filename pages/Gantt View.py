...
from ops_snapshot import DEFAULT_LANE_DEFINITIONS, pull_ops_snapshot
from reserve_calendar_checker import select_reserve_dates_in_range


UTC = timezone.utc
LANE_DEFINITIONS: List[str] = list(DEFAULT_LANE_DEFINITIONS)
...
if st.button("Pull Tail Schedules", type="primary"):
    with st.spinner("Pulling schedules for configured tails..."):
        snapshot = pull_ops_snapshot(config, lane_targets=LANE_DEFINITIONS)
